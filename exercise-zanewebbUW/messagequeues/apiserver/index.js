...
app.post("/order", (req, res) => {
	...
	if (req.body && req.body.itemName && req.body.price) {
		...
		//my garbage
		burgerChannel.sendToQueue(queueName, newOrderMsg)
...
amqp.connect('amqp://localhost:5672', function(err, conn) {
	...
	conn.createChannel(function(err, ch) {
		if (err) {
			console.log("Failed to create NewOrdersChannel from index js in api server");
			process.exit(1);
		}

		burgersChannel = ch;
		burgersChannel.assertQueue(queueName, {durable: true});


		//Dunno what any of this does or if its needed, I just copied it from payment processor 
		burgersChannel.prefetch(1);

		// Set up the worker callback for NewOrders queue
		burgersChannel.consume(queueName, handleNewOrder, {noAck: false});
